...
import { useFullscreenOrientation, type OrientationLockType } from './hooks/use-fullscreen-orientation';
...
export interface PhotoCaptureScreenProps {
  ...
  /** Enable fullscreen mode (default: true on mobile, false on desktop) */
  enableFullscreen?: boolean;
  /** Orientation to lock when in fullscreen (default: 'landscape-primary') */
  preferredOrientation?: OrientationLockType;
  /** Callback when fullscreen/orientation encounters an error */
...
export const PhotoCaptureScreen: React.FC<PhotoCaptureScreenProps> = ({
  ...
  onFlashToggle,
  enableFullscreen,
  preferredOrientation = 'landscape-primary',
  onFullscreenError,
}) => {
  ...
  // Determine default for enableFullscreen based on device type
  const isMobile = permission.browserInfo?.isMobile ?? false;
  const fullscreenEnabled = enableFullscreen !== undefined ? enableFullscreen : isMobile;
  ...
  const {
    ...
    onFullscreenChange: (isFs) => {
      console.debug('[PhotoCaptureScreen] Fullscreen changed:', isFs);
    },
    onOrientationLockError: (err) => {
      console.warn('[PhotoCaptureScreen] Orientation lock failed:', err);
      onFullscreenError?.(err.message);
    },
  });
  ...
  // Request fullscreen when camera is ready
  useEffect(() => {
    if (cameraState === 'ready' && containerRef.current && fullscreenEnabled && !isFullscreen) {
      requestFullscreen(containerRef.current)
        .then(() => {
          console.debug('[PhotoCaptureScreen] Fullscreen + orientation lock successful');
        })
        .catch((err) => {
          console.warn('[PhotoCaptureScreen] Fullscreen request failed:', err);
          // Continue without fullscreen - don't block camera
          onFullscreenError?.(`Fullscreen unavailable: ${err.message}`);
        });
    }
  }, [cameraState, fullscreenEnabled, isFullscreen, requestFullscreen, onFullscreenError]);

  // Exit fullscreen when modal closes
  useEffect(() => {
    if (!isOpen && isFullscreen) {
      exitFullscreen().catch((err) => {
        console.warn('[PhotoCaptureScreen] Exit fullscreen failed:', err);
      });
    }
  }, [isOpen, isFullscreen, exitFullscreen]);
